...
class Solution:
    ...
    def canCompleteCircuit(self, gas, cost):
        init_index = 0
        print(len(gas), len(cost))
        for init_index in xrange(len(gas)):
            if gas[init_index] >= cost[init_index]:
                print "Considering", init_index
                # This is a feasible position
                tank = 0
                found_solution = True
                for ind in xrange(init_index, init_index + len(gas)):
                    i = ind % len(gas)
